...
Вся функциональность класса представлена **членами класса** - **полями** (переменные класса), которые хранят состояние объекта, и **методами**, которые определяют поведение объекта. Например, класс `Person`, который представляет человека, мог бы иметь следующее определение:
